/*
  ...
  1. New Functions
    - `notify_application_submitted` - Sends notification when a new application is submitted
    - `notify_status_updated` - Sends notification when application status changes

  2. New Triggers
    - `on_application_submitted` - Triggers after new application insert
    - `on_application_status_updated` - Triggers after application status update

  3. Changes
    - Creates notification functions that call the edge function
    - Sets up automatic triggers on applications table
*/
...
BEGIN
  ...
  PERFORM
    net.http_post(
      url := current_setting('app.settings.supabase_url') || '/functions/v1/send-email-notification',
